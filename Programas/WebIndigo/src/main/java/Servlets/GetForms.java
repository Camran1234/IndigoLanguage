 ...
 * @author camran1234
 */
@WebServlet(name = "GetForms", urlPatterns = {"/GetForms"})
public class GetForms extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ArrayList<BlockParameter> block = new ArrayList<BlockParameter>();
            String user = new Analysis().getLoggedUser();
            dBIndigo db = new dBIndigo(block);
            ArrayList<Form> forms = db.collectForms(user);
            ArrayList<Parameter> parameters = new ArrayList<>();
            System.out.println("SIZE FORMS: "+forms.size());
            for(int index=0; index<forms.size(); index++){
                Form form = forms.get(index);   
                System.out.println("ID AGREGADO: "+form.getId());
                parameters.add(new Parameter("Text",form.getId()));
            }
            block.add(new BlockParameter(parameters));
            PackageResult pr = new PackageResult();
            pr.printInfoAsBlock(block, out);
            out.flush();
        }
    }
